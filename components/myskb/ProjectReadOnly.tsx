...
const Field: React.FC<{
  ...
  colSpan?: string;
}> = ({ label, value, colSpan = 'sm:col-span-3' }) => (
...
const ProjectReadOnly: React.FC<Props> = ({ data = [] as any }) => {
  ...
  const buildings: Array<any> = Array.isArray(data?.data?.buildings)
    ? data.data.buildings
    : [];
  const ownersDisplay =
  ...
  const businessLabel =
    (data?.business?.name ||
      (typeof data?.business_id === 'number'
        ? formatBusinessFallback(data.business_id)
        : undefined)) ??
    '—';

  const submittedOn = data?.created_at ? new Date(data.created_at) : null;
  const submittedOnStr =
    submittedOn && !isNaN(submittedOn.getTime())
      ? submittedOn.toLocaleString()
      : '—';
  const updatedOn = data?.updated_at ? new Date(data.updated_at) : null;
  const updatedOnStr =
    updatedOn && !isNaN(updatedOn.getTime()) ? updatedOn.toLocaleString() : '—';
  ...
  const totalProcessing = (() => {
    ...
    if (Array.isArray(buildings) && buildings.length > 0) {
      const sum = buildings.reduce(
        (acc: number, b: any) => acc + Number(b?.processingFee || 0),
        0
      );
      return sum;
    }
  ...
  const buildingMetrics = (() => {
    ...
    const openArea = buildings.reduce(
      (acc: number, b: any) => acc + Number(b?.openArea || 0),
      0
    );
    const closeArea = buildings.reduce(
      (acc: number, b: any) => acc + Number(b?.closeArea || 0),
      0
    );
  ...
  const isImage = (p?: string) =>
    !!p && /\.(png|jpe?g|gif|bmp|webp|svg)$/i.test(p);
  ...
  const attachments: Array<{ key: string; label: string; path?: string }> =
    attachmentConfigs
      .map((cfg) => ({
        ...cfg,
        path: data?.data?.[cfg.key] as string | undefined,
      }))
      .filter((a) => !!a.path);
  const paymentRef =
  ...
  return (
    <div className="bg-white shadow rounded-lg p-5">
      ...
      <FormSectionHeader
        ...
        description={t(
          'myskb.project.readOnly.sections.overview.description',
          'Project summary and ownership context'
        )}
      />
      <div className="mt-4">
        <FormRow columns={3}>
          <Field
            ...
            value={
              typeof data?.id === 'number'
                ? formatProjectFallback(data.id)
                : '—'
            }
          />
          <Field
            label={t('myskb.project.readOnly.fields.business', 'Business')}
            value={businessLabel}
          />
          <Field
        ...
        <FormRow columns={3}>
          <Field
            label={t(
              'myskb.project.readOnly.fields.projectOwners',
              'Project Owners'
            )}
          ...
          <Field
            label={t(
              'myskb.project.readOnly.fields.submittedOn',
              'Submitted On'
            )}
            value={submittedOnStr}
          />
          <Field
            label={t(
              'myskb.project.readOnly.fields.lastUpdated',
              'Last Updated'
            )}
            value={updatedOnStr}
          />
      ...
      <FormSectionHeader
        ...
        description={t(
          'myskb.project.readOnly.sections.projectInfo.description',
      ...
      <div className="mt-4">
        <FormRow columns={3}>
          ...
          <Field
            ...
            value={currency(data?.data.processingFees)}
          />
        </FormRow>
        <FormRow columns={1}>
          <Field
      ...
      <div className="mt-4">
        <FormRow columns={3}>
          ...
          <Field
            ...
            value={data?.data?.spesificCondition}
          />
          <Field
            label={t('myskb.project.fields.landArea', 'Land Area (m2)')}
            value={data?.data?.landArea}
          />
        </FormRow>
      </div>
      ...
      <FormSectionHeader
        ...
        description={t(
          'myskb.project.readOnly.sections.usage.description',
          'Submitted buildings and areas'
        )}
      />
      <div className="mt-4">
        {buildings.length === 0 ? (
          <div className="text-sm text-gray-500">
            {t(
              'myskb.project.readOnly.buildings.none',
              'No buildings submitted'
            )}
          </div>
        ) : (
              ...
              <thead>
                <tr>
                  <th className="px-3 py-2 text-left text-xs font-semibold text-gray-700">
                    {t(
                      'myskb.project.readOnly.buildings.headers.name',
                      'Name / Type'
                    )}
                  </th>
                  <th className="px-3 py-2 text-left text-xs font-semibold text-gray-700">
                    {t(
                      'myskb.project.readOnly.buildings.headers.openArea',
                      'Open Area (m2)'
                    )}
                  </th>
                  <th className="px-3 py-2 text-left text-xs font-semibold text-gray-700">
                    {t(
                      'myskb.project.readOnly.buildings.headers.closeArea',
                      'Close Area (m2)'
                    )}
                  </th>
                  <th className="px-3 py-2 text-left text-xs font-semibold text-gray-700">
                    {t(
                      'myskb.project.readOnly.buildings.headers.fee',
                      'Processing Fee'
                    )}
                  </th>
                </tr>
              ...
              <tbody className="divide-y divide-gray-100">
                {buildings.map((b, idx) => (
                  <tr key={idx} className="bg-white">
                    ...
                    <td className="px-3 py-2 text-sm text-gray-900">
                      {Number(b?.closeArea ?? 0)}
                    </td>
                    <td className="px-3 py-2 text-sm text-gray-900">
                      {currency(b?.processingFee)}
                    </td>
                  </tr>
                ))}
                <tr>
                  ...
                  <td className="px-3 py-2 text-sm font-semibold text-gray-900">
                    {Number(buildingMetrics.openArea)}
                  </td>
                  <td className="px-3 py-2 text-sm font-semibold text-gray-900">
                    {Number(buildingMetrics.closeArea)}
                  </td>
                  <td className="px-3 py-2 text-sm font-semibold text-gray-900">
                    {currency(totalProcessing)}
                  </td>
                </tr>
              </tbody>
            </table>
        ...
        {['pending_payment', 'paid'].includes(
          ...
          <>
            ...
            <FormSectionHeader
              title={t(
                'myskb.project.readOnly.sections.payment.title',
                'Payment'
              )}
              description={t(
                'myskb.project.readOnly.sections.payment.description',
                'Payment summary'
              )}
            />
            <div className="mt-4">
              <FormRow columns={3}>
                <Field
                  label={t(
                    'myskb.project.readOnly.fields.paymentStatus',
                    'Payment Status'
                  )}
                  value={
                    <StatusBadge
                ...
                />
                <Field
                  label={t('myskb.project.readOnly.fields.amount', 'Amount')}
                  value={currency(totalProcessing)}
                />
                <Field
                  label={t(
                    'myskb.project.readOnly.fields.reference',
                    'Reference'
                  )}
                  value={paymentRef || '—'}
                />
      ...
      {(stageTimeline.length > 0 || review?.reviewedAt || review?.reason) && (
        <>
          ...
          <FormSectionHeader
            ...
            description={t(
              'myskb.project.readOnly.sections.review.description',
      ...
      <FormSectionHeader
        title={t(
          'myskb.project.readOnly.sections.attachments.title',
          'Attachments'
        )}
        description={t(
          'myskb.project.readOnly.sections.attachments.description',
      ...
      <div className="mt-4 space-y-4">
        {attachments.length === 0 ? (
          <div className="text-sm text-gray-500">
            {t(
              'myskb.project.readOnly.attachments.none',
              ...
              <div key={i} className="border rounded-md p-3 bg-gray-50">
                ...
                {isPdf(att.path) ? (
                  <div>
                    <div className="h-64 w-full border rounded bg-white overflow-hidden">
                      <iframe
                        ...
                        title={t(
                          'myskb.project.readOnly.attachments.previewTitle',
                          '{{label}} preview'
                    ...
                    >
                      {t(
                        'myskb.project.readOnly.attachments.openPdf',
                        'Open PDF in new tab'
                      )}
                    </a>
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    >
                      {t(
                        'myskb.project.readOnly.attachments.openImage',
                        'Open image'
                      )}
                    </a>
                  </div>
                  ...
                  >
                    {t(
                      'myskb.project.readOnly.attachments.openFile',
                      'Open file'
                    )}
                  </a>
                )}

...
export default function Ownership() {
  ...
  const getErrMsg = (
    err: any,
    fallback = t('myskb.ownership.errors.generic', 'Something went wrong')
  ) => {
  ...
  const tabOptions = useMemo<Tab[]>(() => {
    ...
    return [
      { name: 'All', href: '#', label: t('myskb.ownership.tabs.all', 'All') },
      {
        ...
        badgeColor: 'yellow',
        label: t('myskb.ownership.tabs.request', 'Request'),
      },
      {
        name: 'Approved',
        href: '#',
        badge: String(approved),
        badgeColor: 'green',
        label: t('myskb.ownership.tabs.approved', 'Approved'),
      },
    ];
  }, [profiles, t]);
    ...
    listOwnerships({ business_id: businessId, limit: 50, offset: 0 })
      .then(({ data }) => {
        ...
        const mapped: Profile[] = (data || []).map((o: any) => {
          if (o?.email && typeof o?.id === 'number')
            idMap[String(o.email).toLowerCase()] = o.id;
          return {
            name: o.name || o.email,
            email: o.email,
            ...
            imageUrl:
              o.avatar_url || 'https://avatars.githubusercontent.com/u/0?v=4',
      ...
      .catch((e: any) =>
        toast.error(
          getErrMsg(
            e,
            t('myskb.ownership.errors.loadOwners', 'Failed to load owners')
          )
        )
      );
  }, [businessId]);
  ...
  useEffect(() => {
    fetchMyBusinesses()
      .then((data: any) => {
        ...
        const isWithdrawn = (item: any) => {
          ...
          for (const [k, v] of Object.entries(item || {})) {
            if (typeof v === 'string' && /withdrawn/i.test(String(v)))
              return true;
          }
          return false;
        };
      ...
      .catch((err: any) => {
        ...
        toast.error(
          t(
            'myskb.ownership.errors.loadBusinesses',
  ...
  const filteredProfiles = useMemo(() => {
    ...
    return arr.sort((a, b) => {
      ...
      const tA =
        a.createdAt || a.lastSeenDateTime
          ? new Date(a.createdAt || a.lastSeenDateTime!).getTime()
          : 0;
      const tB =
        b.createdAt || b.lastSeenDateTime
          ? new Date(b.createdAt || b.lastSeenDateTime!).getTime()
          : 0;
  ...
  async function doSearch(
    ...
    if (!email) {
      toast(
        t('myskb.ownership.errors.emailRequired', 'Enter an email to invite')
      );
      return;
    }
    if (!businessId) {
      toast(
        t('myskb.ownership.errors.selectBusiness', 'Select a business first')
      );
      return;
    }
    try {
      ...
      setProfiles((prev) => {
        const exists = prev.find(
          (p) => p.email.toLowerCase() === item.email.toLowerCase()
        );
        return exists
          ? prev.map((p) =>
              p.email.toLowerCase() === item.email.toLowerCase() ? item : p
            )
          : [item, ...prev];
      });
      if (typeof o?.id === 'number' && o?.email) {
        setOwnerIdByEmail((m) => ({
          ...m,
          [String(o.email).toLowerCase()]: o.id,
        }));
      }
      if (res.user_exists) {
        toast.success(
          t(
            'myskb.ownership.toast.linked',
            'User linked. Application-only access granted.'
          )
        );
      } else if (res.invited) {
        toast.success(
          t('myskb.ownership.toast.invited', 'Invitation email sent.')
        );
      } else {
        toast.success(
          t('myskb.ownership.toast.processed', 'Invite processed.')
        );
      }
    } catch (e: any) {
      toast.error(
        getErrMsg(e, t('myskb.ownership.errors.invite', 'Failed to invite'))
      );
    }
  }
    ...
    <FormProvider {...methods}>
      <LayoutWithoutSidebar shiftY="-translate-y-0">
        ...
        <InputText
          ...
          type="email"
          placeholder={t(
            'myskb.ownership.searchPlaceholder',
        ...
        <ProfileRow
          ...
          onlineLabel={
            currentTab === 'Request'
              ? t('myskb.ownership.status.pending', 'Pending')
              : undefined
          }
          actions={(profile) => (
            <ProfileActionMenu
                  ...
                  ? [
                      {
                        ...
                        onClick: async () => {
                          const id =
                            ownerIdByEmail[profile.email.toLowerCase()];
                          if (!id) {
                            toast.error(
                              t(
                                'myskb.ownership.errors.missingId',
                          ...
                          try {
                            ...
                            toast.success(
                              t('myskb.ownership.toast.approved', 'Approved')
                            );
                          } catch (e: any) {
                            toast.error(
                              getErrMsg(
                                e,
                                t(
                                  'myskb.ownership.errors.approve',
                                  'Failed to approve'
                                )
                              )
                            );
                  ...
                  : []),
                {
                  ...
                  onClick: async () => {
                    ...
                    try {
                      ...
                      toast.success(
                        t('myskb.ownership.toast.removed', 'Removed')
                      );
                    } catch (e: any) {
                      toast.error(
                        getErrMsg(
                          e,
                          t('myskb.ownership.errors.remove', 'Failed to remove')
                        )
                      );
                    }
                  },

...
export default function PullToRefresh({ onRefresh, threshold = 70, maxPull = 120, children }: Props) {
    ...
    <div style={wrapperStyle}>
      ...
      <div className="pointer-events-none h-0 overflow-visible">
        <div className="mx-auto mt-[-48px] flex h-10 w-10 items-center justify-center rounded-full bg-gray-200 text-gray-600 shadow-md"
             style={{ opacity: Math.min(offset / threshold, 1), transform: `scale(${0.8 + Math.min(offset / threshold, 1) * 0.2})` }}>
          {refreshing ? (

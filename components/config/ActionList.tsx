  ...
  },
  {
    ...
    onClick: async (item: any) => {
      try {
        ...
        const bill_mobile =
          typeof window !== 'undefined'
            ? localStorage.getItem('mobile') || '0123456789'
            : '0123456789';
        ...
        const msg =
          typeof res?.message === 'string' ? res.message : 'Payment submitted';
        toast.success(msg);
      } catch (e: any) {
        const err =
      ...
      onClick: async (item: any) => {
        try {
          ...
          if (status !== 'pending') {
            toast(
              t(
                'myskb.actions.payUnavailable',
                'Payment is available once the application is Pending'
              )
            );
            return;
          }
          ...
          const data = (
            item && typeof item === 'object' ? item.data || item.form || {} : {}
          ) as any;
          ...
          if (
            ...
            amountNum = data.buildings.reduce(
          ...
          if (!amountNum || Number.isNaN(amountNum) || amountNum <= 0) {
            toast.error(
              t(
                'myskb.actions.noFee',
                'No processing fee found for this project.'
              )
            );
            return;
          }
          ...
          const msg =
            typeof res?.message === 'string'
              ? res.message
              : t('myskb.actions.paymentSubmitted', 'Payment submitted');
          toast.success(msg);
        } catch (e: any) {
          const err =
            e?.response?.data?.message ||
            e?.message ||
            t('myskb.actions.paymentFailed', 'Failed to submit payment');
          toast.error(String(err));
        }
      },
    },

        ...
        <Table className="w-full">
          ...
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="w-[28%]">{t('assessment.colAccount', 'Account / Description')}</TableHead>
              <TableHead className="w-[20%]">{t('assessment.colBill', 'Bill No.')}</TableHead>
              <TableHead className="w-[14%]">{t('assessment.colDue', 'Due Date')}</TableHead>
              <TableHead className="w-[14%]">{t('assessment.colAmount', 'Amount')}</TableHead>
              ...
              return (
                <TableRow key={b.id}>
                  ...
                  <TableCell className="font-medium">
                    <div className="truncate">{b.description || t('assessment.unknown', 'Unknown')}</div>
                    <div className={`text-xs mt-1 ${isOverdue(b.due_date) ? 'text-red-600 font-semibold' : 'text-gray-500'}`}>
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-sm font-semibold text-gray-900 truncate">
                    {b.description || t('assessment.unknown', 'Unknown')} {b.bill_no ? `- ${b.bill_no}` : ''}
                  </div>
                  {/* line 2 */}
                  <div className={`text-xs mt-1 ${isOverdue(b.due_date) ? 'text-red-600 font-semibold' : 'text-gray-500'}`}>
                    {t('assessment.due', 'Due')}: {fmtISO(b.due_date)}
                  ...
                  {/* line 3 */}
                  <div className={`text-xs mt-1 ${paid ? 'text-emerald-600 font-semibold' : 'text-gray-700 font-semibold'}`}>

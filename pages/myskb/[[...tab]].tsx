...
const MyskbPage: React.FC = () => {
  ...
  const translatedTabs = useMemo(() => {
    const base =
      !allowedTabs || allowedTabs.length === 0
        ? myskbTabs
        : myskbTabs.filter((tabCandidate) =>
            new Set(allowedTabs.map((x) => x.toLowerCase())).has(
              tabCandidate.name.toLowerCase()
            )
          );
  ...
  const allTabs: Tab[] = useMemo(() => {
    ...
    return translatedTabs.map((tabItem) =>
      tabItem.name.toLowerCase() === 'registration'
        ? {
            ...tabItem,
            badge: t('myskb.tabs.registrationApproved', 'Approved'),
            badgeColor: 'green',
          }
        : tabItem
    );
  ...
  const handleTabChange = (nextTab: Tab) => {
    if (lamApproved && nextTab.name.toLowerCase() === 'registration') {
      toast.success(
        t(
          'myskb.tabs.registrationLocked',
          'LAM approved. Registration is locked.'
        )
      );
      return;
    }
  ...
  useEffect(() => {
    ...
    fetchMyBusinesses()
      .then((list: any[]) => {
        ...
        const saved =
          typeof window !== 'undefined'
            ? localStorage.getItem('myskb_last_business_id')
            : undefined;
        const n = saved ? Number(saved) : NaN;
        const pick =
          !Number.isNaN(n) && n > 0
            ? (list || []).find((b) => Number(b.id) === n)
            : Array.isArray(list)
            ? list[0]
            : null;
      ...
      .catch(() => {
        if (mounted) setLamApproved(false);
      });
    return () => {
      mounted = false;
    };
  ...
  useEffect(() => {
    ...
    const isAllowed =
      allowedSet.size === 0 || allowedSet.has(currentSlug.toLowerCase());
    if (!isAllowed) {
      const target = (allowedTabs[0] || 'application').toLowerCase();
      router.replace(`/myskb/${encodeURIComponent(target)}`, undefined, {
        shallow: true,
      });
    }

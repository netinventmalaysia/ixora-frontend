...
export default function DashboardPage() {
      ...
      try {
        ...
        const [a, c, b, m] = await Promise.all([
          ...
          fetchCompoundOutstanding({ ic })
            .then((d: any) => (Array.isArray(d?.data) ? d.data : Array.isArray(d) ? d : []))
            .catch(() => []),
  ...
  const parseYear = (s?: string) => {
    ...
    if (parts.length >= 3) {
      ...
      return parts[0].length === 4 ? Number(parts[0]) : Number(parts[2]);
    }
    return NaN;
  };
  ...
  const unifiedBills: BillRow[] = useMemo(() => {
    ...
    const ms = (misc || []).map((b) =>
      norm(
        'Misc Bill',
        b.id,
        (b as any).bill_no ?? (b as any).billNo ?? (b as any).no_rujukan ?? String(b.id),
        b.amount,
        b.due_date
      )
    );
  ...
  // After bills are loaded, check paid references
  useEffect(() => {
    const run = async () => {
  ...
  // ===== Yearly aggregation (Paid vs Unpaid) — show ALL years, no selector =====
  const yearlyData = useMemo(() => {
    ...
    // Sort oldest → newest; use .reverse() for newest first if preferred
    ...
    <SidebarLayout>
      ...
      <TextLine>
        {t(
          ...
          'MBMB IXORA is the official digital portal of Majlis Bandaraya Melaka Bersejarah that simplifies citizen and business transactions online.'
        )}
      </TextLine>

      <Spacing size="lg" />

      {/* Paid vs Unpaid by Year (Stacked Bar, real data; all years) */}
      <PaidUnpaidBar
        data={yearlyData}
              ...
              <TableRow key={i}>
                ...
                <TableCell className="text-right">
                  {(() => {
                    ...
                    const isPaid =
                      p && ((p.status || '').toUpperCase() === 'PAID' || (p.status || '').toUpperCase() === 'SUCCESS');
                    if (isPaid) {
                      const href = p?.reference
                        ? `/payment-status/${encodeURIComponent(p.reference!)}`
                        : `/payment-status?bill_no=${encodeURIComponent(b.billNo)}`;
                      return (
                        <a
                          href={href}
                          className="inline-flex items-center px-3 py-1.5 rounded-md bg-indigo-600 text-white hover:opacity-90"
                        >
                          Receipt
                        </a>
                      );
                    ...
                    return (
                      <button
                        ...
                        onClick={() => {
                          ...
                          add(selectable);
                          try {
                            window.dispatchEvent(new Event('ixora:openCheckout'));
                          } catch {}
                        }}
                      >
                        Bayar

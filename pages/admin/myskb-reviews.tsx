...
export default function MySkbReviewsPage() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await listAdminMySkbProjects({
        status,
        limit: 50,
        offset: 0,
      });
  ...
  const onReject = async (p: AdminProjectItem) => {
    ...
    if (
      reason === undefined &&
      !window.confirm('Proceed with rejection without a reason?')
    )
      return;
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="divide-y divide-gray-100">
              {rows.length === 0 ? (
                ...
                rows.map((p) => {
                  const title =
                    (p as any).projectTitle ||
                    (p as any)?.data?.projectTitle ||
                    `#${p.id}`;
                  const created = p.created_at
                    ? new Date(p.created_at).toLocaleString()
                    : '-';
                  const business =
                    (p as any)?.business?.name ||
                  ...
                  return (
                    <tr key={p.id}>
                      ...
                      <td
                        ...
                        onClick={() =>
                          router.push(`/admin/myskb-reviews/${p.id}`)
                        }
                        title="View details"
                      >
                        {title}
                      ...
                      <td className="px-3 py-2 text-sm text-gray-900">
                        <div className="flex gap-2">
                          <button
                            ...
                            className="px-3 py-1.5 rounded-md bg-white ring-1 ring-gray-300 text-gray-900 text-sm"
                          >
                            View
                          </button>
                          {status === 'submitted' ? (

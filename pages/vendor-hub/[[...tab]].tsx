...
import { vendorsTabs } from 'todo/components/data/TabList';
import VendorHub from 'todo/components/vendor-hub/home';
import Registration from 'todo/components/vendor-hub/registration';
import Application from 'todo/components/vendor-hub/application';
import TeamMembers from 'todo/components/vendor-hub/team';
import { teams, logoUrl } from 'todo/components/main/SidebarConfig';
import Billing from 'todo/components/vendor-hub/billing';
...
const VendorsPage: React.FC = () => {
 ...
 const { badgeCounts, loading } = useBadgeCounts({
  ...
  const visibleTabs = vendorsTabs.filter((t) => {
  ...
  const handleTabChange = (t: Tab) => {
    ...
    router.push(`/vendor-hub/${encodeURIComponent(slug)}`, undefined, { shallow: true });
  };
    ...
    if (hasBusinesses === false) {
      ...
      if (forbidden.has(String(currentSlug).toLowerCase())) {
        ...
        if (fallback && fallback !== String(currentSlug).toLowerCase()) {
          router.replace(`/vendor-hub/${encodeURIComponent(fallback)}`, undefined, { shallow: true });
        }
      }
    }

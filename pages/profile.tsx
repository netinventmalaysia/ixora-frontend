...
import { fetchCsrfToken, getUserProfile, updateUser } from "todo/services/api";
...
type UserProfile = {
  ...
  // Optional business fields if frontend still requires them
  city?: string;
  state?: string;
  postalcode?: string;
  country?: string;
...
export default function ProfilePage() {
  ...
  const fetchUser = async () => {
    ...
    try {
      ...
      const res: AxiosResponse<UserProfile> = await getUserProfile(Number(userId));
      const data = res.data;
  ...
  if (!userProfile) return <div className="p-6 text-red-500">Failed to load profile.</div>;

  return (
    <SidebarContent teams={[]} logoUrl={logoUrl} userRole={userRole || 'guest'} username={username} email={email}>
      <FormWrapper
      ...
      >
        <Heading level={2} align="left" bold>Edit Profile</Heading>
        <Spacing size="lg" />

        <FormSectionHeader title="Account Details" description="Update your login and contact information." />
        <Spacing size="lg" />
        ...
        <LineSeparator />
        <FormSectionHeader title="User Information" description="Update your personal profile details." />
        <Spacing size="sm" />
        <PhotoUploadField
          name="profilePicture"
        ...
        {accountType === "business" && (
          <>
            <LineSeparator />
            <FormSectionHeader title="Business Information" description="Update your business or organisation details." />
            <Spacing size="lg" />
            <Toggle
              label=""
